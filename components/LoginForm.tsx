...
export default function LoginForm({ next }: { next: string }) {
  ...
  async function handleEmail(e: React.FormEvent) {
    ...
    if (error) {
      alert(error.message || 'Failed to send link. Check your Supabase Site URL configuration.')
      setLoading(false)
      return
    }
    setSent(true)
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-8">
          <h1 className="text-xl font-bold text-slate-900 mb-1 text-center">Sign in or create your account</h1>
          <p className="text-sm text-slate-500 mb-6 text-center">No password needed.</p>
