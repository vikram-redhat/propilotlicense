...
export default async function LicencePage({ params }: { params: Promise<{ licence: string }> }) {
  ...
  if (examType === 'Composite' && licence.toLowerCase() === 'cpl') {
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--clr-surface)', color: 'var(--clr-text)' }}>
        ...
        <section style={{ borderBottom: '1px solid var(--clr-border)', padding: '24px 20px' }} className="sm:px-9 lg:px-[60px]">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--clr-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontFamily: 'var(--font-outfit),sans-serif', fontWeight: 800, fontSize: 13, flexShrink: 0, textAlign: 'center', lineHeight: 1.2 }}>
              COM
            </div>
            <div>
              <h1 style={{ fontFamily: 'var(--font-outfit),sans-serif', fontSize: 20, fontWeight: 700, color: 'var(--clr-text)', letterSpacing: '-0.3px' }}>
                Composite Licence Conversion
              </h1>
              <p style={{ fontSize: 13, color: 'var(--clr-text-med)', marginTop: 2 }}>Foreign licence conversion · 2 papers</p>
            </div>
          </div>
        </section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4" style={{ maxWidth: 680 }}>
            ...
            {byCode['REG'] && (
              <Link
                ...
                className="hover:border-[var(--clr-primary)] transition-colors"
                style={{ background: 'var(--clr-surf-alt)', borderRadius: 13, border: '1px solid var(--clr-border)', padding: 18, display: 'flex', flexDirection: 'column', gap: 8, textDecoration: 'none' }}
              >
                <div style={{ width: 38, height: 38, borderRadius: 9, background: 'var(--clr-pri-light)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <SubjectIcon name={byCode['REG'].icon_name} size={20} className="text-[var(--clr-primary)]" />
                </div>
                <div style={{ fontFamily: 'var(--font-outfit),sans-serif', fontWeight: 600, fontSize: 13, color: 'var(--clr-text)', lineHeight: 1.3 }}>
                  {byCode['REG'].name}
                </div>
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--clr-surface)', color: 'var(--clr-text)' }}>
      ...
      <section style={{ borderBottom: '1px solid var(--clr-border)', padding: '24px 20px' }} className="sm:px-9 lg:px-[60px]">
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--clr-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontFamily: 'var(--font-outfit),sans-serif', fontWeight: 800, fontSize: 15, flexShrink: 0 }}>
            {licenceUpper}
          </div>
          <div>
            <h1 style={{ fontFamily: 'var(--font-outfit),sans-serif', fontSize: 20, fontWeight: 700, color: 'var(--clr-text)', letterSpacing: '-0.3px' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--clr-text-med)', marginTop: 2 }}>{dashboardNote}</p>
          </div>
        </div>
      </section>
      ...
      <main className="flex-1 px-5 sm:px-9 lg:px-[60px] py-8">
        {subjects.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '64px 0', color: 'var(--clr-text-med)' }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {subjects.map(subject => (
              <Link
                ...
                className="hover:border-[var(--clr-primary)] transition-colors"
                style={{ background: 'var(--clr-surf-alt)', borderRadius: 13, border: '1px solid var(--clr-border)', padding: 16, display: 'flex', flexDirection: 'column', gap: 8, textDecoration: 'none' }}
              >
                <div style={{ width: 38, height: 38, borderRadius: 9, background: 'var(--clr-pri-light)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <SubjectIcon name={subject.icon_name} size={20} className="text-[var(--clr-primary)]" />
                </div>
                <div style={{ fontFamily: 'var(--font-outfit),sans-serif', fontWeight: 600, fontSize: 13, color: 'var(--clr-text)', lineHeight: 1.3 }}>
                  {subject.name}
                </div>
