...
import { IconPlus, IconSparkles, IconEdit, IconTrash, IconCheck } from '@tabler/icons-react'
...
export default function AdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    let query = supabase
      .from('questions')
      .select('*, subject:subjects(name,code), topic:topics(name)', { count: 'exact' })
  ...
  // Stats
  const [stats, setStats] = useState({ total: 0, pending: 0, published: 0, ai: 0 })
  useEffect(() => {
    async function loadStats() {
      const { data } = await supabase.from('questions').select('active, source_type')
      if (!data) return
      setStats({
        total: data.length,
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {q.subject && (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 font-medium">
                          {(q.subject as { name: string; code: string }).name}
                        </span>
                      )}
